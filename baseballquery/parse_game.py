...
class ParseGame:
    def __init__(self, game: dict, convert_id: ConvertMLBAM):  # type: ignore
        ...
        for player, _ in home_players.items():  # type: ignore
        ...
        self.away_starting_pitcher = self.convert_id.mlbam_to_retro(self.game["gameData"]["probablePitchers"]["away"]["id"])  # type: ignore
        self.home_starting_pitcher = self.convert_id.mlbam_to_retro(self.game["gameData"]["probablePitchers"]["home"]["id"])  # type: ignore
    ...
    def parse(self):
        # TODO: Figure out how to add the manfred runner. I assume that can be done within ParsePlateAppearance (since it'll probably be in the event thing)
        ...
        for plate_appearance in self.game["liveData"]["plays"]["allPlays"]:  # type: ignore
            ...
            if len(plate_appearance["playEvents"]) == 0:  # type: ignore
                ...
                continue
            pa = ParsePlateAppearance(plate_appearance, self.game_id, self.away_team, self.home_team, self.starting_lineup_away, self.starting_lineup_home, self.away_starting_pitcher, self.home_starting_pitcher, self.convert_id, runners, runner_resp_pit_id)  # type: ignore
            pa.parse()
